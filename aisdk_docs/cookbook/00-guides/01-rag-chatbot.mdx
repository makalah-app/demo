...
In this project, you will build a agent that will only respond with information that it has within its . The agent will be able to both store and retrieve information. This project has many interesting use cases from customer support through to building your own second brain!
...
3. Create an agent
4. Give the agent tools to query / create resources for it’s 
...
export async function POST(req: Request) {
  ...
  const result = streamText({
    model: openai('gpt-4o'),
    system: `You are a helpful assistant. Check your  before answering any questions.
...
Let’s see how you can create a tool to give the model the ability to create, embed and save a resource to your agents’ .
...
export async function POST(req: Request) {
  ...
  const result = streamText({
    ...
    tools: {
      addResource: tool({
        description: `add a resource to your .
          If the user provides a random piece of knowledge unprompted, use this tool without asking for confirmation.`,
        inputSchema: z.object({
          content: z
            .string()
            .describe('the content or resource to add to the '),
        }),
...
The model can now add and embed arbitrary information to your . However, it still isn’t able to query it. Let’s create a new tool to allow the model to answer questions by finding relevant information in your .
...
export async function POST(req: Request) {
  ...
  const result = streamText({
    ...
    tools: {
      ...
      getInformation: tool({
        description: `get information from your  to answer questions.`,
        inputSchema: z.object({
...
Congratulations, you have successfully built an AI agent that can dynamically add and retrieve information to and from a . Throughout this guide, you learned how to create and store embeddings, set up server actions to manage resources, and use tools to extend the capabilities of your agent.
